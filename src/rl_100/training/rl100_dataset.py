...
class RL100TransitionDataset(Dataset):
    def __init__(
        ...
        self.has_success = "next.success" in self.dataset.features
    ...
    def _infer_episode_success(self, ep_idx: int, ep_start: int, ep_end: int) -> bool:
        ...
        for abs_idx in range(ep_start, ep_end):
            ...
            if self.has_reward and float(np.asarray(frame[REWARD]).reshape(-1)[0]) > 0.0:
                return True
            if self.has_success and bool(np.asarray(frame["next.success"]).reshape(-1)[0]):
                return True
        return False
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        for step_idx in range(self.n_action_steps):
            ...
            elif self._episode_success[ep_idx] and query_idx == ep_end - 1:
                reward = 1.0
            ...
            elif query_idx == ep_end - 1:
                frame_done = True
            done = done or frame_done

            if self.has_success:
                success = success or bool(np.asarray(frame["next.success"]).reshape(-1)[0])
            else:
